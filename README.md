...
**Keeping the video.** The pipeline only ever needs the audio, but subtitles belong to a picture: with `WHISPER_URL_VIDEO_ENABLED` (on by default when URL download is on) a request may add `keep_video=true` (and an optional `video_max_height`) to also fetch the best video + best audio merge — the preview's `video_ladder` lists the site's video rungs best first in yt-dlp's own order (YouTube's enhanced-bitrate "Premium" 1080p ranks above plain 1080p, exactly as `bv*+ba` would pick), each with its yt-dlp `format_id`s, bitrate, an exact or approximate size (`bytes_approx`: fragmented HLS/DASH streams list no size and a peak bitrate), the container the merge produces (MP4 for H.264/AAC, MKV for VP9/AV1/Opus) and an over-cap flag. The download fetches the rung's exact format ids (generic best-under-cap as the fallback), so the file is what the card priced; a request names its rung with `video_format=<format_id>` or caps it with `video_max_height`. For approximate rungs the server learns each site's actual/estimated byte ratio per protocol family in the rates ledger (`stage_rates.json`, key `dlratio|<extractor>|<m3u8|dash|https>|`) and scales the next preview's estimate by it. A site that names no rankable formats but serves a video file gets one "Best available" rung. The video is fetched after the audio, alongside the transcription, never on the GPU path; the response carries `source_video_media_id` once it is done or `source_video_pending` while it still runs (keep polling the progress id for `video.state`). `POST /v1/audio/url-media/video` fetches it later for a run that did not keep it. Every path is bounded by one ceiling, `MEDIA_MAX_BYTES` (10 GB, uploads included), the retention dir by `RETAINED_MEDIA_MAX_BYTES`, and video downloads by `URL_VIDEO_DOWNLOAD_TIMEOUT_S` and `URL_VIDEO_RATE_PER_MIN`. Retained media (audio and video) lives under `URL_MEDIA_DIR` for `URL_MEDIA_TTL_S`; a video download stages inside that dir so the final move is a rename, but the retention volume must hold the video AND the transcription's own temp copy of the audio.

**Deployment: outbound network isolation.** A client picks the URL, so the server can be pointed at things it can reach and the client cannot — a LAN service, the cloud metadata endpoint at `169.254.169.254`. The application blocks that on its own: one address policy (`faster_whisper_backend/core/net_policy.py`) is applied to **every** fetch on this path — the direct-media probe, the thumbnail fetch, yt-dlp's metadata probe and the yt-dlp download subprocess — on **hop 0 and every redirect hop**, with the resolved IP **pinned** for the connection (so a second DNS answer cannot move the target) and only `http(s)` spoken. yt-dlp's own opener is covered by the guard in [`ytdlp_plugins/`](ytdlp_plugins/), installed in-process and in the download subprocess; **if it cannot be installed, link downloads are refused** rather than run unguarded (look for `[url-dl] SSRF guard active` at startup). **Outbound proxy:** set `http_proxy`/`https_proxy` in the server's environment and every fetch on this path goes through it; the proxy itself may sit on a private address (it is operator-configured, so the address policy trusts that hop), while the link's target is still gated by name on every hop. The server must be able to resolve the target's hostname itself — a proxy-only network where local DNS cannot resolve public names refuses every link rather than fetching unchecked. SOCKS proxies are refused (the policy could not see the target).
...
.git-blame-ignore-revs     The move commits of the package reorganisation; Forgejo/GitHub honour it, locally: git config blame.ignoreRevsFile .git-blame-ignore-revs
