...
_FIELD_GROUPS: list[tuple[str, list[tuple[str | None, list[str]]]]] = [
    ...
    ("Access & sessions", [
        (None, [
            ...
            "CORS_ALLOW_ORIGINS", "TRUSTED_ORIGINS",
...
function buildSectionNav() {
  ...
  navRow.innerHTML = '';
  // The dedicated /settings/pipeline page renders a single section and has no
  // use for the settings section-nav — leave the strip empty (CSS
  // `.section-nav:empty { display:none }` hides it) rather than listing sections
  // that live on the main /settings page.
  if (PIPELINE_ONLY) { refreshSectionToggleAllLabel(); return; }
